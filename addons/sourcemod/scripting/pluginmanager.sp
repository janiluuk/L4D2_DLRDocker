...
{
	name = "Plugin Manager",
	author = "R-Hehl modded by Jameless",
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_loadplugin", Command_LoadIt, ADMFLAG_SLAY)
	RegAdminCmd("sm_unloadplugin", Command_UnLoadIt, ADMFLAG_SLAY)
}
...
public Action:Command_LoadIt(client, args)
{
	new Handle:pluginsdir = OpenDirectory("addons/sourcemod/plugins/disabled");
	new String:name[64];
	...
	new String:newpath[128]
	new String:oldpath[128]
	Format(oldpath, sizeof(oldpath), "addons/sourcemod/plugins/disabled/%s.smx",name);
	Format(newpath, sizeof(newpath), "addons/sourcemod/plugins/%s.smx",name);
	if(FileExists(newpath))
	{
	ReplyToCommand(client,"[Plugin Manager] This plugin does not allow you to overwrite plugins already in the directory")
	CloseHandle(pluginsdir)
	return Plugin_Handled;
	}
	...
	ReplyToCommand(client,"[Plugin Manager] %s has been loaded successfully",name)
	}
	else
	{
	ReplyToCommand(client,"[Plugin Manager] %s is not in the disabled directory",name)
	}
	CloseHandle(pluginsdir)
	return Plugin_Handled;
}
public Action:Command_UnLoadIt(client, args)
{
	new Handle:pluginsdir = OpenDirectory("addons/sourcemod/plugins");
	new String:name[64];
	GetCmdArg(1, name, sizeof(name))
	...
	new String:newpath[128]
	new String:oldpath[128]
	Format(oldpath, sizeof(oldpath), "addons/sourcemod/plugins/%s.smx",name);
	Format(newpath, sizeof(newpath), "addons/sourcemod/plugins/disabled/%s.smx",name);
	if(FileExists(newpath))
	{
	ReplyToCommand(client,"[Plugin Manager] This plugin does not allow you to overwrite plugins already in the directory")
	CloseHandle(pluginsdir)
	...
	RenameFile(newpath,oldpath)
	ReplyToCommand(client,"[Plugin Manager] %s has been unloaded successfully",name)
	}
	else
	{
	ReplyToCommand(client,"[Plugin Manager] %s is not in the plugins directory",name)
	}
	CloseHandle(pluginsdir)
	return Plugin_Handled;
}