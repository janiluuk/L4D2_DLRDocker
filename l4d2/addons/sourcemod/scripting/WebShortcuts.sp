...
public bool:IsTeamFortress2() {
	return (g_iGameMode & Game_TF2);
}

public bool:IsLeftForDead() {return (g_iGameMode & Game_L4D);}
public bool:GoLargeOrGoHome() {return (IsTeamFortress2() && (g_iGameMode & Big_MOTD));}
...
public OnPluginStart()
{
	...
	new Handle:cvarVersion = CreateConVar("webshortcutsredux_version", PLUGIN_VERSION, PLUGIN_DESCRIPTION, FCVAR_NOTIFY);
...
public Duck_OnPluginStart()
{
	...
	new iSDKVersion = GetEngineVersion();
...
stock Duck_DoReplacements(iClient, String:sUrl[256], iUrlBits, String:sTitle[256], iTitleBits) /* Huge thanks to Psychonic */
{
	if (iUrlBits & Flag_Steam_ID || iTitleBits & Flag_Steam_ID)
	{
		...
		if (GetClientAuthId(iClient, AuthId_Engine, sSteamId, sizeof(sSteamId)))
		{
