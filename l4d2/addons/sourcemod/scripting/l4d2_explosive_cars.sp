...
public OnPluginStart()
{
	...
	g_cvarPanicChance = CreateConVar("l4d2_explosive_cars_panic_chance", "100", "Chance that the cars explosion might call a horde (%)", FCVAR_NONE);
	...
	g_cvarCarFlyMax = CreateConVar("l4d2_explosive_cars_car_max_height", "2500", "Maximum power when the car is launched", FCVAR_NONE);
	g_cvarCarFlyMin = CreateConVar("l4d2_explosive_cars_car_min_height", "1000", "Minimum power when the car is launched", FCVAR_NONE);
	g_cvarSelfHurt = CreateConVar("l4d2_explosive_cars_fire_selfdamage", "0", "Should the car get damage from its own fire before it explodes? This will eventually end in a explosion", FCVAR_NONE);
	g_cvarInfernoNum = CreateConVar("l4d2_explosive_cars_inferno_count", "2", "How many extra infernos (fire) should be created upon explosion?", FCVAR_NONE);
	g_cvarFireMulti = CreateConVar("l4d2_explosive_cars_fire_multiplier", "2.42", "Multiplier of the damage received by the car from fire (Note: Normal is 8 damage per 0.5 secs)", FCVAR_NONE);
	g_cvarExploMulti = CreateConVar("l4d2_explosive_cars_explosion_multiplier", "45.67", "Multiplier of the damage received by the car from explosions (Pipe bombs, grenade launcher, propane tanks, etc) (Note: Normal is 15-20 damage per shot)", FCVAR_NONE);
